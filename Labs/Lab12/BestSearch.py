...
def bfs(maze):
    ...
    # Create a heap of nodes to explore. Initially push the starting node onto the heap.
    # Tuples have the format (score, row, col) where score is always zero. Score indicates
    # the priority which determines which node to explore.
    toExpand = []
    toExpand.append((0, start_r, start_c))

    ### Main BFS Execution Loop ###
    # While there are still nodes left on the heap to explore, process them one at a time.
    while len(toExpand) > 0:
        ...
        if maze[cur_r][cur_c] != '*':
            if not visited[cur_r-1][cur_c]:  # up
                heapq.heappush(toExpand, manhattanProject(cur_r-1, cur_c))
            if not visited[cur_r][cur_c-1]:  # left
                heapq.heappush(toExpand, manhattanProject(cur_r, cur_c-1))
